        ...
        <>
            ...
            <div className="max-w-[1535px] mx-auto flex flex-col lg:gap-20 gap-6 py-5">
                <section className="relative bg-black max-lg:flex max-lg:flex-col max-lg:gap-6">
                    {resTitleDetail?.trailer_thumbnail ?
                        <img src={resTitleDetail?.trailer_thumbnail || imgs.imgDefault} alt={resTitleDetail?.title} className="w-full aspect-[16/9] lg:opacity-30" onError={handleImgError} />
                        :
                        <img src={resTitleDetail?.poster || imgs.imgDefault}
                            alt={resTitleDetail?.title || "No image available"} className="aspect-[3/4]" />
                    }
                    <div className="flex items-center lg:absolute lg:top-1/2 lg:left-1/2 lg:-translate-x-1/2 lg:-translate-y-1/2">
                        <div className="lg:w-[1025px] w-full lg:px-10 relative flex flex-col gap-5">
                    ...
                    <div className="lg:absolute lg:-bottom-[50px] lg:left-1/2 lg:-translate-x-1/2 flex lg:gap-6 gap-4 text-cyan-950 lg:text-lg text-sm grid grid-cols-5 lg:w-[90%] w-full">
                        <div className="grid justify-center gap-2 items-center rounded-[10px] bg-cyan-500 backdrop-blur-[10px] p-5 border-gray-800 shadow-sm shadow-cyan-300/50 transition-all duration-300 ease hover:-translate-y-[3px]">
                            <span className="lg:text-3xl text-2xl font-bold text-cyan-950 text-center ">{resTitleSeasons.length}</span>
                            Seasons
                        </div>
                        <div className="grid justify-center gap-2 items-center rounded-[10px] bg-cyan-500 backdrop-blur-[10px] p-5 border-gray-800 shadow-sm shadow-cyan-300/50 transition-all duration-300 ease hover:-translate-y-[3px]">
                            <span className="lg:text-3xl text-2xl font-bold text-cyan-950 text-center ">{resTitleEpisodes.length}</span>
                            Episodes
                        </div>
                        <div className="grid justify-center gap-2 items-center rounded-[10px] bg-cyan-500 backdrop-blur-[10px] p-5 border-gray-800 shadow-sm shadow-cyan-300/50 transition-all duration-300 ease hover:-translate-y-[3px]">
                            <span className="lg:text-3xl text-2xl font-bold text-cyan-950 text-center ">{resTitleDetail?.user_rating ?? 0}/10</span>
                            User Score
                        </div>
                        <div className="grid justify-center gap-2 items-center rounded-[10px] bg-cyan-500 backdrop-blur-[10px] p-5 border-gray-800 shadow-sm shadow-cyan-300/50 transition-all duration-300 ease hover:-translate-y-[3px]">
                            <span className="lg:text-3xl text-2xl font-bold text-cyan-950 text-center ">{resTitleDetail?.critic_score ?? 0}%</span>
                            Critic Score
                        </div>
                        <div className="grid justify-center gap-2 items-center rounded-[10px] bg-cyan-500 backdrop-blur-[10px] p-5 border-gray-800 shadow-sm shadow-cyan-300/50 transition-all duration-300 ease hover:-translate-y-[3px] transition-all duration-300 ease hover:-translate-y-[3px]">
                            <span className="lg:text-3xl text-2xl font-bold text-cyan-950 text-center ">{resTitleDetail?.runtime_minutes ?? 0}</span>
                            <span className="text-center">Runtime Minutes</span>
                        </div>
                    </div>
                </section>
                <div className="grid lg:grid-cols-[1fr_300px] gap-6">
                    <section className="flex flex-col gap-6 self-start w-full">
                                        ...
                                        resTitleEpisodes.map((res, index) => (
                                            <>
                                                <button
                                                    ...
                                                    className={`${resTitleSeasons.length > 0 ? selectSeasons === res.season_id ? "" : "hidden" : ''} flex gap-4 rounded-[10px] transition-all duration-100 ease group hover:border-l-[3px] hover:border-l-cyan-300 pl-[1px]`}>
                                                    <img src={res.thumbnail_url} alt={res.name} className="w-[150px] aspect-[2/1] rounded-[10px] group-hover:scale-102" onError={handleImgError} />
                                                    <div className="flex flex-col gap-2 w-full">
                                                        <div className="flex justify-between">
                                                            <h3 className="text-lg text-white"><strong>{index + 1}- {res.name}</strong></h3>
                                                            <span className="text-sm py-1 px-4 rounded-[10px] bg-cyan-500/10 text-cyan-300 border-[1px] border-cyan-500/10">{res.runtime_minutes ?? 0} minutes</span>
                                                        </div>
                                                        <p className="text-sm text-start text-white/70">{res.overview}</p>
                                                        ...
                                                        <div className="flex gap-2 text-sm">
                                                            ...
                                                            <div className="w-[1px] h-full bg-gray-500"></div>
                                                            <span><strong>{convertDateEpisodes(res.release_date)}</strong></span>
                                                    ...
                                                    <>
                                                        <div className="flex gap-2">
                                                            {res.sources.map((sour) => (
                                                                <button className={`flex gap-2 self-end text-sm text-white text-start items-center px-2 h-[30px] w-fit rounded-[10px] transition-all duration-300 ease`} style={styleColor(res.id)}>
                                                                    {sour.name}
                                                                </button>
                                                            ))}
                                                        </div>
                        ...
                        {resTitleCast.length > 0 &&
                            <div className="w-full items-center border-[1px] border-gray-800 p-5 rounded-[10px] bg-gray-900 shadow-lg transition-all duration-300 ease">
                                <div className="flex justify-between text-white css-next items-center w-full transition-all duration-300 ease">
                    ...
                    <aside className="flex flex-col gap-6 self-start max-lg:row-start-1">
                        <div className="relative group w-full max-lg:hidden">
                            <img src={resTitleDetail?.poster || imgs.imgDefault}
                                alt={resTitleDetail?.title || "No image available"} className="aspect-[3/4] w-full rounded-[10px] transition-all duration-300 ease shadow-lg shadow-gray-900 group-hover:scale-103 group-hover:opacity-70 group-hover:shadow-cyan-300/50" onError={handleImgError} />
                            <button
                                onClick={() => {
                                    setModalOpen(true);
                                }}
